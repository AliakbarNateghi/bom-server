...
def create_god_permissions(apps, schema_editor):
    ...
    ScopeMatrixFieldPermission = apps.get_model(
        "component", "ScopeMatrixFieldPermission"
    )
    ScopeMatrixComponent = apps.get_model("component", "ScopeMatrix")
    ...
    field_names = [
        field.name
        for field in ScopeMatrixComponent._meta.get_fields()
        if field.name not in ["id", "deleted", "deletable"]
    ]
    field_permissions = [
        ...
        for i in range(1, ScopeMatrixComponent.objects.count() + 1)
        for field_name in field_names
    ]
    ScopeMatrixFieldPermission.objects.bulk_create(field_permissions)
