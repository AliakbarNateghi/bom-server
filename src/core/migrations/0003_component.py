    ...
    for index, row in BOMdf.iterrows():
        bom_components.append(
            BomComponent(
                ...
                internal_or_external_outsourcing=check_NaN(row["Internal / External outsourcing"]),
                ...
                manufacturing_responsible_department=check_NaN(row[
                    "Manufacturing Responsible Department"
                ]),
                designing_responsible_department=check_NaN(row[
                    "Designing Responsible Department"
                ]),
                usage_on_other_engines=check_NaN(row["USAGE ON OTHER ENGINES"]),
                manufacturing_parts_category=check_NaN(row["MANUFACTURING PARTS Category"]),
                scope_matrix_category=check_NaN(row["Scope Matrix Category"]),
                requires_manufacturing_or_supplying_for_reassembly=check_NaN(row[
                ...
                discarded_during_disassembly=check_NaN(row["DISCARDED DURING DISSASSEMBLY"]),
                expendables=True if row["Expendables"] == "yes" else False,
                discarded_or_unusable_according_to_docs=check_NaN(row[
                    "Discarded/Unusable According To Docs"
                ]),
                destroyed_for_analysis=check_NaN(row["Destroyed For Analysis"]),
                rejected_by_qc_or_inspection=check_NaN(row["Rejected by QC/Inspection"]),
                class_size_or_weight_as_required=check_NaN(row["Class Size/Weight As Required"]),
